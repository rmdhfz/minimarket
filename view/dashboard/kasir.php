...
<head>
	...
	<title>Kasir | Aplikasi Kasir &dot; Hafiz Ramadhan - 191011402923</title>
...
<body>
	<div class="container" id="box">
		<div class="col-sm-12">
			<h1>Selamat datang di Menu Kasir 
		...
		<div class="table-responsive">
			<table id="table-kasir" class="table table-striped">
				<thead>
					<tr>
						<th>No</th>
						<th>NIK</th>
				...
				<tbody>
					<?php 
						...
						$con = new Database();
						$kasir = $con->mysqli->query("SELECT * FROM hafizramadhan_kasir");
						$no = 1;
						while($value = mysqli_fetch_array($kasir)){
							echo "
							<tr>
									<td>".$no++."</td>
									<td>".$value['nik']."</td>
	...
	<div id="modal-kasir" tabindex="-1" role="dialog" class="modal fade modal-flex">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="container-fluid">
						<form id="form-kasir" method="post">
							...
							<div class="container">
								...
								<div class="form-group row">
									<label class="form-label col-sm-4">Nama Kasir</label>
									<input type="text" id="nama" name="nama" class="col-sm-8 form-control" required placeholder="masukan nama kasir">
								</div>
								<div class="form-group row">
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("#form-kasir").submit(function(event) {
			event.preventDefault();
			if (confirm("Apakah data yang Anda input sudah benar ?")) {
				let id = $("#id").val() === "" ? url = "handler/kasir/create.php" : url = "handler/kasir/update.php";
				$.post(baseurl + url, $(this).serializeArray(), function(res) {
					if (res.status) {
						alert("kasir berhasil dinput");
		...
		$("#table-kasir").on('click', '#edit', function(event) {
			event.preventDefault();
			const id = $(this).data('id');
			if (id) {
				$.post(baseurl + 'handler/kasir/read.php', {nik: id}, function(res) {
					if (res) {
						const data = res.data;
						if (data) {
		...
		$("#table-kasir").on('click', '#delete', function(event) {
			event.preventDefault();
			if (confirm("Apakah Anda yakin ingin menghapus data ini ?")) {
				const id = $(this).data('id');
				if (id) {
					$.post(baseurl + 'handler/kasir/delete.php', {nik: id}, function(res) {
						if (res) {
							window.location.reload(false);
						}
					});
